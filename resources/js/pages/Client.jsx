...
function displayChecks(checks) {
  const arr = Array.isArray(checks) ? checks : (typeof checks === "string" ? checks.split(/[·,]/).map(x => x.trim()).filter(Boolean) : []);
  return arr.map(c => CHECK_ABBREV[c] || String(c)).join(", ");
}
...
export default function Client() {
  ...
  const counts = {
    all:           cases.length,
    pending:       cases.filter(c => c.status === "pending" || c.status === "in-progress").length,
    dueToday:      0,
    dueSoon:       0,
    overdue:       0,
    completed:     cases.filter(c => c.status === "completed").length,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ position: "relative", width: "300px" }}>
          ...
          <input 
            ...
            style={{ width: "100%", padding: "8px 12px 8px 32px", borderRadius: "6px", border: "1px solid #e2e8f0", fontSize: "13px", outline: "none" }}
          />
        </div>
        <div style={{ display: "flex", gap: "10px" }}>
          <button 
            onClick={() => { setSearch(""); setCaseTypeFilter("All"); setCheckTypeFilter("All"); setTatStatusFilter("All"); setDateFilter("month"); }}
            style={{ background: "#f8fafc", border: "1px solid #e2e8f0", padding: "8px 16px", borderRadius: "6px", fontSize: "13px", fontWeight: 600, color: "#475569", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px" }}
          >
            🔄 Reset
          </button>
          <button 
            style={{ background: "#1d4ed8", border: "none", padding: "8px 20px", borderRadius: "6px", fontSize: "13px", fontWeight: 600, color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px" }}
          >
            ⚡ Apply Filters
          </button>
        </div>
    ...
    <div style={{ background: "#fff", borderRadius: "8px", overflow: "hidden", border: "1px solid #e2e8f0" }}>
          ...
          {loading ? (
            ...
            rows.map((c, i) => (
              <tr key={c.case_id || i} style={{
                ...
                <td style={{ padding: "14px", color: "#475569", maxWidth: "260px", lineHeight: "1.4" }}>
                  {displayChecks(c.checks) || "Background Verification, Identity Check, Address Verification"}
                </td>
                <td style={{ padding: "14px", color: "#475569", whiteSpace: "nowrap" }}>
                  {c.created_at ? new Date(c.created_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }) : "12 Sep 2025"}
                </td>
                <td style={{ padding: "14px", color: "#475569", whiteSpace: "nowrap" }}>{formatTatDisplay(c)}</td>
                <td style={{ padding: "14px", whiteSpace: "nowrap" }}><StatusBadge status={c.status} /></td>
                <td style={{ padding: "14px", whiteSpace: "nowrap" }}>
                  <button style={{ background: "#f8fafc", border: "1px solid #e2e8f0", padding: "6px 12px", borderRadius: "6px", cursor: "pointer", fontSize: "12px", fontWeight: 600, color: "#334155", display: "inline-flex", alignItems: "center", gap: "4px" }}>
                    📥 Download
                  </button>
                </td>
                <td style={{ padding: "14px", whiteSpace: "nowrap" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    <button onClick={() => { navigate(`/Client?tab=pending`); setSelectedCase(c); }} style={{ background: "#eff6ff", border: "1px solid #bfdbfe", color: "#1d4ed8", padding: "6px 12px", borderRadius: "6px", cursor: "pointer", fontSize: "12px", fontWeight: 600 }}>
                      View Details
                    </button>
                    <span style={{ cursor: "pointer", color: "#94a3b8", fontWeight: "bold", fontSize: "16px" }}>⋮</span>
                  </div>
                </td>
              </tr>
            ))
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", borderTop: "1px solid #f1f5f9", background: "#fff" }}>
        ...
        <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
          <button style={{ border: "1px solid #e2e8f0", background: "#fff", width: "28px", height: "28px", borderRadius: "4px", cursor: "pointer", color: "#64748b" }}>&lt;</button>
          <button style={{ border: "none", background: "#2563eb", color: "#fff", width: "28px", height: "28px", borderRadius: "4px", fontWeight: 600, fontSize: "13px", cursor: "pointer" }}>1</button>
          <button style={{ border: "1px solid #e2e8f0", background: "#fff", width: "28px", height: "28px", borderRadius: "4px", cursor: "pointer", color: "#64748b" }}>&gt;</button>
        </div>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <section id="content">
        <Header />
        <main>
          <div className="dash-wrper" style={{ padding: "20px", background: "#f8fafc" }}>
            {/* Top Page Header */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              <div>
                <h2 style={{ fontSize: "20px", fontWeight: "700", color: "#1e2761", margin: 0 }}>Active Cases</h2>
                <p style={{ fontSize: "13px", color: "#64748b", margin: "2px 0 0 0" }}>View and manage all cases that are currently in progress.</p>
              </div>
