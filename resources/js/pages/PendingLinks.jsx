...
// Turns ["employment","education"] into "Employment, Education Verification".
// A single check reads as "Employment Verification"; several checks are
// joined and share one trailing "Verification" rather than repeating it.
function formatCheckType(checks) {
  if (!Array.isArray(checks) || checks.length === 0) return "—";
  const labels = checks.map((c) => c.charAt(0).toUpperCase() + c.slice(1));
...
export default function PendingLinks() {
  ...
  const fetchLinks = () => {
    ...
    return fetch(`${API_URL}/api/candidate-links`, {
      headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
    })
      .then((r) => r.json())
      .then((data) => setRawLinks(data.links || []))
      .catch(() => setError("Failed to load candidate links."))
  ...
  // "Pending Links" only shows links the candidate hasn't submitted yet —
  // pending (awaiting) or expired. A submitted link belongs on the case
  // itself, not in this queue.
  const links = useMemo(() => {
    return rawLinks
      .filter((l) => l.displayStatus !== "submitted")
      .map((l) => ({
        id: l.id,
        caseId: l.caseId || "—",
        candidateName: l.candidateName || "Unnamed Candidate",
        caseType: "New Case", // not tracked separately on the backend yet
        checkType: formatCheckType(l.checks),
        sentDate: formatDateTime(l.createdAt),
        linkExpiry: formatDate(l.expiresAt),
        status: l.displayStatus === "expired" ? "Expired" : "Awaiting Candidate",
        link: l.link,
      }));
  }, [rawLinks]);

  // Stat cards are computed from the full set (including submitted links),
  // since "Completion Rate" needs the submitted count too.
  const stats = useMemo(() => {
    const totalPending = rawLinks.filter((l) => l.displayStatus !== "submitted").length;
    const sentToday = rawLinks.filter((l) => isToday(l.createdAt)).length;
    const awaiting = rawLinks.filter((l) => l.displayStatus === "pending").length;
    const expired = rawLinks.filter((l) => l.displayStatus === "expired").length;
    const submitted = rawLinks.filter((l) => l.displayStatus === "submitted").length;
    const completionRate = rawLinks.length
      ? Math.round((submitted / rawLinks.length) * 100)
      : 0;
    return { totalPending, sentToday, awaiting, expired, completionRate };
  }, [rawLinks]);

  // Distinct check-type labels for the filter dropdown, derived from
  // whatever checks actually appear across the current links.
  const checkTypeOptions = useMemo(() => {
    const set = new Set();
    links.forEach((l) => set.add(l.checkType));
  ...
  // Resend — reuses POST /candidate-links/{id}/send, the same route the
  // link-generator dashboard already uses. Defaults to Email since that's
  // the only channel this table lets the user trigger.
  const handleResend = async (id, candidateName) => {
    const token = localStorage.getItem("token");
    setResendingId(id);

    try {
      const res = await fetch(`${API_URL}/api/candidate-links/${id}/send`, {
        method: "POST",
        headers: {
      ...
      alert(`Link resent to ${candidateName}`);
    } catch (err) {
  ...
  const handleResendAll = async () => {
    const targets = filteredLinks.filter((l) => l.status !== "Expired");
    if (targets.length === 0) {
      alert("No active pending links to resend.");
      return;
    }
    ...
    try {
      await Promise.all(
        targets.map((l) =>
          fetch(`${API_URL}/api/candidate-links/${l.id}/send`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
      ...
      alert(`Resent verification links to ${targets.length} pending candidate(s)!`);
    } catch (err) {
      alert("Some links failed to resend. Please try again.");
    } finally {
    ...
    <>
      ...
      <section id="content">
        ...
        <main style={{ padding: "24px", backgroundColor: "#f8fafc" }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "16px" }}>
            <button
              style={{ ...styles.resendAllBtn, opacity: resendingAll ? 0.7 : 1, cursor: resendingAll ? "not-allowed" : "pointer" }}
              onClick={handleResendAll}
              disabled={resendingAll}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
              {resendingAll ? "Resending…" : "Resend All Pending Links"}
            </button>
          </div>

          {/* KPI Stat Cards — derived from live candidate-links data */}
          <div style={styles.statsGrid}>
            <div style={{ ...styles.statCard, borderColor: "#f59e0b" }}>
              <div style={{ ...styles.iconCircle, backgroundColor: "#fef3c7", color: "#d97706" }}>
                🔗
              ...
              <div>
                <h3 style={{ ...styles.statNumber, color: "#d97706" }}>{stats.totalPending}</h3>
                <p style={styles.statLabel}>Total Pending Links</p>
              </div>
            </div>
            ...
            <div style={{ ...styles.statCard, borderColor: "#3b82f6" }}>
              ...
              <div>
                <h3 style={{ ...styles.statNumber, color: "#2563eb" }}>{stats.sentToday}</h3>
                <p style={styles.statLabel}>Links Sent Today</p>
              </div>
            </div>
            ...
            <div style={{ ...styles.statCard, borderColor: "#8b5cf6" }}>
              ...
              <div>
                <h3 style={{ ...styles.statNumber, color: "#7c3aed" }}>{stats.awaiting}</h3>
                <p style={styles.statLabel}>Awaiting Candidate</p>
              </div>
            </div>
            ...
            <div style={{ ...styles.statCard, borderColor: "#ef4444" }}>
              ...
              <div>
                <h3 style={{ ...styles.statNumber, color: "#dc2626" }}>{stats.expired}</h3>
                <p style={styles.statLabel}>Expired Links</p>
              </div>
            </div>
            ...
            <div style={{ ...styles.statCard, borderColor: "#10b981" }}>
              ...
              <div>
                <h3 style={{ ...styles.statNumber, color: "#059669" }}>{stats.completionRate}%</h3>
                <p style={styles.statLabel}>Completion Rate</p>
              </div>
            </div>
          </div>
          ...
          <div style={styles.tableCard}>
            <table style={styles.table}>
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={9} style={{ ...styles.td, textAlign: "center", color: "#94a3b8" }}>Loading…</td></tr>
                ) : error ? (
                  <tr><td colSpan={9} style={{ ...styles.td, textAlign: "center", color: "#dc2626" }}>{error}</td></tr>
                ) : currentItems.length === 0 ? (
                  <tr><td colSpan={9} style={{ ...styles.td, textAlign: "center", color: "#94a3b8" }}>No pending links.</td></tr>
                ) : (
                  currentItems.map((item, index) => {
                    const statusStyle = getStatusBadgeStyle(item.status);
                    const isExpired = item.status === "Expired";
                        ...
                        <td style={{ ...styles.td, textAlign: "right" }}>
                          <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end", alignItems: "center" }}>
                            <button
                              ...
                              onClick={() => handleResend(item.id, item.candidateName)}
                              disabled={resendingId === item.id}
                            >
                              📥 {resendingId === item.id ? "Sending…" : "Resend Link"}
                            </button>
