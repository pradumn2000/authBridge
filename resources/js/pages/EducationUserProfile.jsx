...
export default function EducationUserProfile() {
  ...
  useEffect(() => {
    ...
    // Reuses GET /api/cases (same list EducationCheck.jsx already fetches)
    // and finds this case by id.
    fetch(`${API_URL}/api/cases`, {
      ...
      .then((data) => {
        ...
        if (!found) {
          ...
          setCaseData(found);
          setVerifier(found.assigned_verifier || "");
        }
      })
  ...
  const educationFields = caseData?.check_details?.education?.fields;
  const qualifications = Array.isArray(educationFields?.qualifications) ? educationFields.qualifications : [];
  const selectedQualification = qualifications[selectedIndex] || null;
  const overallStatusLabel = STATUS_LABELS[caseData?.status] || "Unassigned";
  ...
  if (loading) {
    return (
      <div className="eup-app-layout">
        ...
        <div className="eup-main-wrapper" id="content">
          <Header />
          <div className="eup-container" style={{ textAlign: "center", padding: "60px 0", color: "#94a3b8" }}>
  ...
  if (error || !caseData) {
    return (
      <div className="eup-app-layout">
        ...
        <div className="eup-main-wrapper" id="content">
          <Header />
          <div className="eup-container" style={{ textAlign: "center", padding: "60px 0" }}>
            <p style={{ color: "#dc2626", fontWeight: 600, marginBottom: "12px" }}>{error || "Case not found."}</p>
            <button className="eup-back-btn" onClick={() => navigate("/EducationCheck")} style={{ justifyContent: "center" }}>
              ‹ Back to Education Cases
            </button>
          </div>
        </div>
        ...
        <div className="eup-container">
          ...
          <div className="eup-top-grid">
            ...
            <div className="eup-card eup-profile-header">
              <img
                src="https://via.placeholder.com/60"
                alt={educationFields?.candidate_name || caseData.candidate}
                className="eup-avatar"
              />
              <div className="eup-profile-info">
                <div className="eup-title-badge">
                  <h3>{educationFields?.candidate_name || caseData.candidate}</h3>
                  <span className="eup-badge-active">{overallStatusLabel}</span>
                </div>
                <div className="eup-details-grid">
                  <div className="eup-meta-item">
                    <label>Case ID</label>
                  ...
                  <div className="eup-meta-item">
                    <label>Candidate ID</label>
                    <span>{educationFields?.candidate_id || "—"}</span>
                  </div>
                  {/* Date of Birth isn't collected anywhere in the Education
                      Details form, so shown honestly as "—" */}
                  <div className="eup-meta-item">
                    <label>Date of Birth</label>
                    <span>—</span>
                  </div>
                  <div className="eup-meta-item">
                    <label>Mobile Number</label>
                    <span>{educationFields?.mobile_number || "—"}</span>
                  </div>
                  <div className="eup-meta-item">
                    <label>Email Address</label>
                    <span>{educationFields?.email_address || "—"}</span>
                  </div>
                  {/* Current Location isn't collected either */}
                  <div className="eup-meta-item" style={{ gridColumn: "span 2" }}>
                    <label>Current Location</label>
                    <span>—</span>
                  </div>
            ...
            <div className="eup-card">
              ...
              <div className="eup-overview-row">
                <span>📅 Case Created On</span>
                <strong>{caseData.created_at || "—"}</strong>
              </div>
              <div className="eup-overview-row">
                <span>⏰ Expected Completion</span>
                <strong>{caseData.due_date || "—"}</strong>
              </div>
              <div className="eup-overview-row">
                <span>⏱️ SLA</span>
                <strong>
                  {caseData.check_tat?.education
                    ? `${caseData.check_tat.education.working_days || caseData.check_tat.education.calendar_days || 0} Days`
                    : "—"}
                </strong>
              </div>
            </div>

            {/* Select Verifier — reassignment isn't wired to an endpoint yet;
                this reflects the case's currently assigned verifier but the
                dropdown list itself is still a static placeholder set. */}
            <div className="eup-card">
              <h4>Select Verifier</h4>
              <select
                className="eup-select-input"
              ...
              >
                <option value="">
                  {caseData.assigned_verifier ? caseData.assigned_verifier : "Unassigned"}
                </option>
                <option value="Amit Kumar">Amit Kumar (Education Verifier)</option>
          ...
          <div className="eup-main-layout">
            ...
            <div className="eup-card">
              ...
              {qualifications.length === 0 ? (
                <p style={{ fontSize: "12px", color: "#94a3b8" }}>No qualifications saved yet for this case.</p>
              ) : (
                <>
                  <div className="eup-table-container">
                    <table className="eup-table">
                      ...
                      <tbody>
                        {qualifications.map((item, idx) => (
                          ...
                          >
                            <td>{idx + 1}</td>
                            <td><strong>{item.qualificationType || "—"}</strong></td>
                            <td>{item.courseStream || "—"}</td>
                            <td>{item.instituteUniversity || "—"}</td>
                            {/* Only exists when "Year of Passing" fee mode
                                was selected for this qualification */}
                            <td>
                              {item.verificationFeesBy === "Year of Passing" && (item.fromYOP || item.toYOP)
                                ? `${item.fromYOP || "?"}–${item.toYOP || "?"}`
                                : "—"}
                            </td>
                            <td>
                              {/* No per-qualification status is tracked —
                                  this reflects the overall case status */}
                              <span className="eup-badge-pending">{overallStatusLabel}</span>
                            </td>
                            <td>
                              <button className="eup-action-icon" title="View Details" onClick={(e) => { e.stopPropagation(); setSelectedIndex(idx); }}>
                                👁️
                              </button>
                            </td>
                          </tr>
                  ...
                  <p style={{ fontSize: "11px", marginTop: "12px" }}>Showing 1 to {qualifications.length} of {qualifications.length} qualifications</p>
                </>
              )}
            </div>
            ...
            <div className="eup-card eup-right-panel">
              <button className="eup-close-panel">✕</button>
              <p style={{ fontSize: "11px" }}>Selected Qualification</p>
              <h4 style={{ fontSize: "15px", margin: "2px 0 10px 0" }}>
              ...
              {/* Details Tab Content */}
              {activeRightTab === "details" && selectedQualification && (
                <div style={{ display: "flex", flexDirection: "column", gap: "8px", fontSize: "12px" }}>
                  <div className="eup-overview-row"><span>Specialization</span><strong>{selectedQualification.specialization || "—"}</strong></div>
                  <div className="eup-overview-row"><span>Board / University</span><strong>{selectedQualification.boardUniversity || "—"}</strong></div>
                  <div className="eup-overview-row"><span>Scope</span><strong>{selectedQualification.studyType || "—"}</strong></div>
                  <div className="eup-overview-row"><span>Verification Fees By</span><strong>{selectedQualification.verificationFeesBy || "—"}</strong></div>
                  <div className="eup-overview-row"><span>Verification Fees (₹)</span><strong>{selectedQualification.universityFees || "—"}</strong></div>
                  <div className="eup-overview-row"><span>GST</span><strong>{selectedQualification.gst || "—"}</strong></div>
                  <div className="eup-overview-row"><span>Total Amount</span><strong>{selectedQualification.totalAmount || "—"}</strong></div>
                </div>
              )}

              {/* Documents Tab Content — no upload wired yet, so this is
                  shown honestly empty rather than with fake filenames */}
              {activeRightTab === "documents" && (
                <div>
                  <div className="eup-docs-header">
                    <strong style={{ fontSize: "12px" }}>Documents Provided by Candidate</strong>
                  ...
                  <p style={{ fontSize: "11px", color: "#94a3b8", marginBottom: "16px" }}>
                    No documents uploaded yet for this qualification.
                  </p>
              ...
              {/* Verification History Tab — no event log fetched here yet;
                  GET /cases/{caseId}/timeline already exists and could feed
                  this if you want real history instead of an empty state */}
              {activeRightTab === "history" && (
                <p style={{ fontSize: "11px", color: "#94a3b8" }}>No verification history loaded.</p>
              )}
            </div>
          </div>
