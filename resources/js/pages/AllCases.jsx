...
// // import { computeCheckStatus } from "../src/checkFormsConfig";
// //
// // [... original commented-out legacy version preserved as-is above your live code ...]
...
// // Where each check type's badge navigates to when clicked, from
// // CheckwiseGrid below. Replaces the old CheckDetailForm popup.
// const CHECK_ROUTE = {
...
//         <div
//           key={chk}
//           onClick={() => navigate(`${CHECK_ROUTE[chk] || "/AllCases"}?caseId=${encodeURIComponent(c.case_id)}`)}
//           style={{
//             display: "flex", alignItems: "center", justifyContent: "space-between",
//             padding: "9px 0", borderBottom: isLast ? "none" : "1px solid #f1f5f9",
//             cursor: "pointer", gap: "8px",
...
// ── Extract check keys for a case row (same logic used elsewhere) ───────────
function getChecksArrayGlobal(c) {
  if (Array.isArray(c.checks)) return c.checks;
  if (typeof c.checks === "string") return c.checks.split(/[·,]/).map(x => x.trim()).filter(Boolean);
  return [];
}

// ── Per-check Working Days / Calendar Days lists, pulled from check_tat.
//    Accepts both { working_days, calendar_days } and camelCase shapes
//    (matches the checkTatPayload shape sent from AddCase.jsx).
function getCheckTatDays(c) {
  const checks = getChecksArrayGlobal(c);
  const tat = c.check_tat && typeof c.check_tat === "object" ? c.check_tat : {};
  const working  = checks.map(k => Number(tat[k]?.working_days  ?? tat[k]?.workingDays)  || 0);
  const calendar = checks.map(k => Number(tat[k]?.calendar_days ?? tat[k]?.calendarDays) || 0);
  return { working, calendar };
}

function formatDaysList(list, suffix) {
  return list.length > 0 ? `${list.join(", ")} ${suffix}` : "—";
}
...
// ── Due date = created_at + max calendar days across selected checks.
//    Falls back to any explicit due_date / created_at if we can't compute.
function computeDueDate(c) {
  const maxDays = getMaxCalendarDays(c);
  const base = c.created_at ? new Date(c.created_at) : null;
  if (!maxDays || !base || isNaN(base.getTime())) return c.due_date || c.created_at || "—";
            ...
            {activeDetailTab === "overview" && (
              <div>
                {[
                  ...
                  { label: "Priority", value: selectedCase.priority || "Normal" },
                  { label: "TAT", value: formatTatDisplay(selectedCase) },
                  { label: "Working Days", value: formatDaysList(getCheckTatDays(selectedCase).working, "WD") },
                  { label: "Calendar Days", value: formatDaysList(getCheckTatDays(selectedCase).calendar, "CD") },
                  { label: "Due Date", value: computeDueDate(selectedCase) },
                  { label: "Created", value: selectedCase.created_at || "—" },
                  { label: "Amount", value: `₹${(selectedCase.total_amount || 0).toLocaleString()}` },
                ].map(r => (
                  <div key={r.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center",
                      ...
                      <table className="bk-custom-table">
                        ...
                        <tbody>
                          {filtered.length === 0 ? (
                            ...
                            filtered.map((row) => {
                              const isSelected = selectedCase?.case_id === row.case_id;
                              const maxDays = getMaxCalendarDays(row);
                              return (
                                <tr
                                  key={row.case_id}
                                  onClick={() => {
                                ...
                                >
                                  ...
                                  <td style={{ fontSize: "13px", whiteSpace: "nowrap" }}>
                                    {formatDaysList(getCheckTatDays(row).working, "WD")}
                                  </td>
                                  <td style={{ fontSize: "13px", whiteSpace: "nowrap" }}>
                                    {formatDaysList(getCheckTatDays(row).calendar, "CD")}
                                  </td>
                                  <td style={{ fontSize: "12px", color: "#94a3b8", whiteSpace: "nowrap" }}>
                                    {computeDueDate(row)}
                                    {maxDays > 0 && (
                                      <div style={{ fontSize: "10px", color: "#0d9488", fontWeight: 700 }}>
                                        Max {maxDays}d
                                      </div>
                                    )}
                                  </td>
