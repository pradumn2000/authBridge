...
const [generatingCandidateLink, setGeneratingCandidateLink] = useState(false);
const [candidateLinkError, setCandidateLinkError] = useState("");

const generateCandidatePortalLink = async () => {
  setGeneratingCandidateLink(true);
  setCandidateLinkError("");
  try {
    const token = localStorage.getItem("token");
    const res = await fetch(`${API_URL}/api/cases/${encodeURIComponent(caseId)}/share-link`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ checks: form.checks }),
    });
    const data = await res.json();
    if (!res.ok) {
      setCandidateLinkError(data.message || "Failed to generate candidate link.");
      return;
    }
    setCandidatePortalLink(data.url);
  } catch {
    setCandidateLinkError("Server error. Please try again.");
  } finally {
    setGeneratingCandidateLink(false);
  }
};
