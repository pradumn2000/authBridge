...
// // ── Default turnaround time per check type — split into Working Days
// //    and Calendar Days, matching the Satyapan UI. Both start blank;
// //    admin fills in the actual TAT per case.
// const DEFAULT_CHECK_TAT = {
//   employment: { workingDays: '', calendarDays: '' },
//   education:  { workingDays: '', calendarDays: '' },
//   address:    { workingDays: '', calendarDays: '' },
//   database:   { workingDays: '', calendarDays: '' },
//   criminal:   { workingDays: '', calendarDays: '' },
//   drug:       { workingDays: '', calendarDays: '' },
//   court:      { workingDays: '', calendarDays: '' },
// };

// // ── FALLBACK_CHECK_TYPES — the original hardcoded 7 built-in checks.
// //    Used as the initial value of checkTypeDefs (state) and by the
// //    module-level helper functions below (getEmptyForm, buildCheckRates,
// //    buildCheckTats, caseToForm), which run before the API fetch
// //    resolves and therefore can't read component state. Once
// //    /api/check-types/all responds, checkTypeDefs is replaced with the
// //    live list — including any custom check types (e.g. "Social Media")
// //    an admin has added via Add Check Type — and everything in the
// //    component body that renders check types reads from checkTypeDefs.
// const FALLBACK_CHECK_TYPES = [
...
// // ── Case onboarding source — drives whether the client field/billing mode
...
// // ── Build the TAT map, same pattern as buildCheckRates. Accepts either the
// //    new { workingDays, calendarDays } shape or a legacy single number
// //    (applied to both fields) for older client contracts.
...
//       const mapped = CHECK_KEY_ALIASES[key] || key;
//       if (!(mapped in tats)) return;
//       if (val && typeof val === "object") {
//         tats[mapped] = {
//           workingDays:  Number(val.workingDays ?? val.working_days) || 0,
//           calendarDays: Number(val.calendarDays ?? val.calendar_days) || 0,
//         };
//       } else {
//         const n = Number(val) || 0;
//         tats[mapped] = { workingDays: n, calendarDays: n };
//       }
//     });
//     return tats;
//   }
...
//     ? user.agreedChecks.map(k => CHECK_KEY_ALIASES[k] || k).filter(k => FALLBACK_CHECK_TYPES.some(ct => ct.key === k))
//     : [];

//   return {
//     // ── Onboarding source: "client" (default) or "candidate". Candidate
//     //    sourcing makes the client field & billing mode optional below.
//     caseSource: "client",
...
//   return {
//     caseSource:      c.case_source || fallback.caseSource,
...
//       // Bulk rows default to client-sourced onboarding unless specified.
//       case_source:     row.case_source || "client",
//     });
//   });
//   return { rows, errors };
// }
...
//   // ── Check TAT (turnaround time) — split into Working Days and Calendar
//   //    Days per check. Admin can edit these live per case; everyone else
//   //    sees the fixed default / contract TAT as-is.
//   const [tats, setTats] = useState(() => buildCheckTats(user));

//   // ── Live check-type catalogue — starts from the 7 built-in fallback
//   //    checks and is replaced by whatever /api/check-types/all returns
//   //    (built-ins + any custom check types an admin has added, e.g.
//   //    "Social Media"). Everything that renders the check-types list in
//   //    this form reads from checkTypeDefs, not the hardcoded array, so a
//   //    newly added check type shows up here automatically.
//   const [checkTypeDefs, setCheckTypeDefs] = useState(FALLBACK_CHECK_TYPES);
...
//   const isCandidateSourced = form.caseSource === "candidate";
...
//   // ── Fetch the live check-type catalogue. Runs for everyone (not just
//   //    admin) so a client or allocator also sees any custom check type
//   //    that's been added and made available. Existing rate/TAT values for
//   //    the 7 built-ins are left untouched; a new custom check type gets
//   //    seeded with its configured default_rate / default_working_days /
//   //    default_calendar_days the first time it loads.
//   useEffect(() => {
//     let cancelled = false;
//     const token = localStorage.getItem("token");
...
//       .then(data => {
//         if (cancelled) return;
//         const active = (data.checkTypes || []).filter(ct => ct.is_active !== false);
//         if (active.length === 0) return;

//         setCheckTypeDefs(active.map(ct => ({ key: ct.key, label: ct.label })));

//         setRates(prev => {
//           const next = { ...prev };
//           active.forEach(ct => {
//             if (!(ct.key in next)) next[ct.key] = Number(ct.default_rate) || 0;
//           });
//           return next;
//         });
...
//         // this case, if the API returns a per-check TAT breakdown. Handles
//         // both the new { working_days, calendar_days } shape and a legacy
//         // single number (applied to both fields).
...
//                 next[mapped] = {
//                   workingDays:  Number(v.workingDays ?? v.working_days) || 0,
//                   calendarDays: Number(v.calendarDays ?? v.calendar_days) || 0,
//                 };
//               } else {
//                 const n = Number(v) || 0;
//                 next[mapped] = { workingDays: n, calendarDays: n };
//               }
//             });
//             return next;
//           });
...
//   // ── Update a single check's TAT — field is "workingDays" or "calendarDays" ──
//   const setTat = (key, field, value) => {
//     const num = Number(value);
//     setTats(p => ({
//       ...p,
//       [key]: { ...p[key], [field]: Number.isFinite(num) && num >= 0 ? num : 0 },
//     }));
//   };

//   const set = (field, value) => setForm(p => ({ ...p, [field]: value }));

//   // ── Switching onboarding source. Going to "candidate" doesn't wipe the
//   //    client selection (admin/allocator may still want to tag a client),
//   //    it just relaxes the requirement. Going back to "client" doesn't
//   //    auto-fill anything either — the requirement simply re-applies.
//   const setCaseSource = (key) => set("caseSource", key);
...
//   //    Admin/staff are never restricted. Built from the LIVE catalogue
//   //    (checkTypeDefs), so a newly added custom check type is picked up
//   //    automatically once it's fetched.
...
//     ? checkTypeDefs.filter(ct => agreedCheckKeys.includes(ct.key) || form.checks.includes(ct.key))
//     : checkTypeDefs;

//   // ── Per-check TAT display — shows each selected check's day count
//   //    individually (e.g. "5, 6, 7") rather than summing, separately for
//   //    Working Days and Calendar Days. A sum would overstate the real
//   //    timeline since checks typically run in parallel, not back-to-back.
//   const overallWorkingDaysValues  = form.checks.map(k => Number(tats[k]?.workingDays) || 0);
//   const overallCalendarDaysValues = form.checks.map(k => Number(tats[k]?.calendarDays) || 0);
//   const overallWorkingDaysDisplay  = overallWorkingDaysValues.length  > 0 ? overallWorkingDaysValues.join(", ")  : "";
//   const overallCalendarDaysDisplay = overallCalendarDaysValues.length > 0 ? overallCalendarDaysValues.join(", ") : "";
...
//   // Number(c.overall_tat ?? c.tat). Using the longest single check's
//   // calendar days here (real elapsed time, not the sum) so that aggregate
//   // stays meaningful.
//   const overallTat = overallCalendarDaysValues.length > 0 ? Math.max(...overallCalendarDaysValues) : 0;

//   // ── TAT payload actually sent to the API — scoped to selected checks only.
//   //    `tats` state carries an entry per known check key (empty string
//   //    defaults for anything the user hasn't touched), so sending the raw
//   //    object as-is pushes empty strings for unselected checks and trips
//   //    the backend's "must be a number" validation. Only the checks in
//   //    form.checks are relevant, and everything in there gets coerced to
//   //    a real number.
//   const checkTatPayload = Object.fromEntries(
//     form.checks.map(k => [k, {
//       working_days:  Number(tats[k]?.workingDays)  || 0,
//       calendar_days: Number(tats[k]?.calendarDays) || 0,
//     }])
//   );
...
//     // ── Client & billing mode are only mandatory when the case is being
//     //    onboarded via a client. Candidate self-onboarding skips both.
//     if (!isCandidateSourced) {
...
//       // ── Candidate-sourced cases fall back to sensible defaults for the
//       //    fields that are now optional in the form: a placeholder client
//       //    name, and candidate-pays billing if none was picked.
//       const resolvedClientName = form.clientName || (isCandidateSourced ? "Self (Candidate)" : "");
//       const resolvedBillingMode = form.billingMode || (isCandidateSourced ? "prepaid_candidate" : "");
...
//                       <span>Onboarded Via</span>
//                       <strong>{isCandidateSourced ? "Candidate" : "Client"}</strong>
//                     </div>
//                     <div className="ac-success-meta-row">
...
//                     </div>
//                     <div className="ac-success-meta-row">
//                       <span>Client</span><strong>{form.clientName || (isCandidateSourced ? "— (Candidate self-onboarded)" : "—")}</strong>
//                     </div>
//                     <div className="ac-success-meta-row">
//                       <span>Billing</span>
...
//                       <strong>{overallWorkingDaysDisplay ? `${overallWorkingDaysDisplay} WD` : "—"}</strong>
//                     </div>
//                     <div className="ac-success-meta-row">
//                       <span>Calendar Days</span>
//                       <strong>{overallCalendarDaysDisplay ? `${overallCalendarDaysDisplay} CD` : "—"}</strong>
//                     </div>
...
//                         {checkTypeDefs.find(t => t.key === c)?.label || c}
//                       </span>
//                     ))}
//                   </div>
...
//             {/* ══ 00 — Case Onboarding Source ══ */}
//             <div className="ac-card" style={{ marginBottom: "20px" }}>
//               <div className="ac-card-header">
//                 <span className="ac-num">00</span>
//                 <h3>Onboarding Source <span className="ac-req">*</span></h3>
//               </div>
//               <div className="ac-billing-grid" style={{ gridTemplateColumns: "repeat(2, 1fr)" }}>
//                 {CASE_SOURCES.map(src => {
//                   const active = form.caseSource === src.key;
//                   return (
//                     <button key={src.key} type="button"
//                       className={`ac-billing-tile ${active ? "ac-billing-active" : ""}`}
//                       style={active ? { borderColor: src.color, background: `${src.color}10` } : {}}
//                       onClick={() => setCaseSource(src.key)}>
//                       <div className="ac-billing-tile-top">
//                         <span className="ac-billing-dot" style={{ background: active ? src.color : "#cbd5e1" }} />
//                         <span className="ac-billing-label" style={active ? { color: src.color } : {}}>
//                           {src.label}
//                         </span>
//                       </div>
//                       <p className="ac-billing-desc">{src.desc}</p>
//                     </button>
//                   );
//                 })}
//               </div>
//               {isCandidateSourced && (
//                 <div className="ac-billing-info-row" style={{ marginTop: "12px" }}>
//                   <span className="ac-billing-info-icon">ℹ</span>
//                   <span>Candidate is self-onboarding. The client field and billing mode below are optional — you can still tag a client if relevant.</span>
//                 </div>
//               )}
//             </div>
...
//                         can still tag a client for reporting purposes if they want. */}
//                     <div className="ac-field">
//                       <label className="ac-label">
//                         Client{" "}
//                         {!isCandidateSourced
//                           ? <span className="ac-req">*</span>
...
//                                 ? "No clients found"
//                                 : isCandidateSourced
//                                 ? "— No Client (Candidate self-onboarding) —"
//                                 : "— Select Client —"}
//                             </option>
//                             {clients.map((c) => (
...
//                     <h3>
//                       Billing Mode {!isCandidateSourced && <span className="ac-req">*</span>}
//                       {isCandidateSourced && <span className="ac-optional">(optional)</span>}
//                     </h3>
...
//                         <span className="ac-billing-info-icon">ℹ</span>
//                         <span>No billing mode is required — this case is being self-onboarded by the candidate.</span>
//                       </div>
//                     ) : (
...
//                       <thead>
//                         <tr>
//                           <th rowSpan={2} style={{ textAlign: 'left', padding: '12px', verticalAlign: 'bottom' }}>Check Type</th>
//                           <th rowSpan={2} style={{ textAlign: 'left', padding: '12px', verticalAlign: 'bottom' }}>Amount</th>
//                           <th colSpan={2} style={{ textAlign: 'center', padding: '12px 12px 4px' }}>TAT</th>
//                         </tr>
//                         <tr>
//                           <th style={{ textAlign: 'left', padding: '0 12px 12px', fontSize: '0.72rem', color: '#94a3b8', fontWeight: 700 }}>Working Days</th>
//                           <th style={{ textAlign: 'left', padding: '0 12px 12px', fontSize: '0.72rem', color: '#94a3b8', fontWeight: 700 }}>Calendar Days</th>
//                         </tr>
//                       </thead>
//                       <tbody>
//                         {visibleCheckTypes.map(ct => {
//                           const active = form.checks.includes(ct.key);
//                           const workingVal  = Number(tats[ct.key]?.workingDays)  || 0;
//                           const calendarVal = Number(tats[ct.key]?.calendarDays) || 0;

//                           // TAT color logic based on reference image
//                           const getTatColor = (days) => {
//                             if (days <= 1) return '#2ecc71'; // Green
//                             if (days <= 3) return '#f1c40f'; // Yellow
...
//                                     <span className="ac-rate-prefix">₹</span>
//                                     <span className="ac-rate-value">{rates[ct.key] || 0}</span>
//                                   </div>
//                                 )}
//                               </td>
...
//                                   <div className="ac-check-rate-edit" title="Working days" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
//                                     <span 
//                                       style={{ 
//                                         height: '10px', 
//                                         width: '10px', 
//                                         backgroundColor: getTatColor(workingVal), 
//                                         borderRadius: '50%', 
...
//                                       value={tats[ct.key]?.workingDays ?? ''}
//                                       onChange={(e) => setTat(ct.key, "workingDays", e.target.value)}
//                                       style={{ width: '50px' }}
//                                     />
//                                     <span className="ac-rate-suffix">d</span>
//                                   </div>
//                                 ) : (
//                                   <div className="ac-check-rate-display" title="Working days" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
//                                     <span 
//                                       style={{ 
//                                         height: '10px', 
//                                         width: '10px', 
...
//                                       }} 
//                                     />
//                                     <span className="ac-rate-value">{workingVal}</span>
//                                     <span className="ac-rate-suffix">d</span>
//                                   </div>
//                                 )}
//                               </td>
...
//                               <td style={{ padding: '12px' }} onClick={(e) => e.stopPropagation()}>
//                                 {isAdminUser || !isClientUser ? (
//                                   <div className="ac-check-rate-edit" title="Calendar days" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
//                                     <span 
//                                       style={{ 
//                                         height: '10px', 
//                                         width: '10px', 
//                                         backgroundColor: getTatColor(calendarVal), 
//                                         borderRadius: '50%', 
//                                         display: 'inline-block' 
//                                       }} 
//                                     />
...
//                                       min="0"
//                                       className="ac-rate-input ac-tat-input"
//                                       value={tats[ct.key]?.calendarDays ?? ''}
//                                       onChange={(e) => setTat(ct.key, "calendarDays", e.target.value)}
//                                       style={{ width: '50px' }}
//                                     />
//                                     <span className="ac-rate-suffix">d</span>
//                                   </div>
//                                 ) : (
//                                   <div className="ac-check-rate-display" title="Calendar days" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
//                                     <span 
//                                       style={{ 
//                                         height: '10px', 
//                                         width: '10px', 
...
//                                       }} 
//                                     />
//                                     <span className="ac-rate-value">{calendarVal}</span>
//                                     <span className="ac-rate-suffix">d</span>
//                                   </div>
//                                 )}
//                               </td>
...
//                       Est. TAT: <strong>{overallWorkingDaysDisplay ? `${overallWorkingDaysDisplay} WD` : "—"}</strong>
//                       {" / "}
//                       <strong>{overallCalendarDaysDisplay ? `${overallCalendarDaysDisplay} CD` : "—"}</strong>
//                     </span>
//                     {isAdminUser && (
...
//                 {/* Summary strip */}
//                 {(isCandidateSourced || form.billingMode) && form.checks.length > 0 && (
//                   <div className="ac-summary-strip"
//                     style={activeBilling
//                       ? { borderColor: activeBilling.color, background: `${activeBilling.color}0d` }
//                       : { borderColor: "#0d9488", background: "#0d94880d" }}>
...
//                     <div className="ac-summary-row">
//                       <span>Billing</span>
//                       <strong style={{ color: activeBilling?.color }}>{activeBilling?.label || "— (not required)"}</strong>
//                     </div>
//                     <div className="ac-summary-row">
//                       <span>Checks</span>
...
//                     candidate_name, candidate_email, mobile, position, checks, billing_mode, client_name, case_source
//                   </code><br />
...
//                     For <code>case_source</code>, use <code>client</code> or <code>candidate</code> (defaults to <code>client</code> if omitted).
//                   </span>
//                 </div>
...
//                             {["Candidate", "Email", "Checks", "Billing", "Source"].map(h => (
...
//                               <td style={{ padding: "7px 10px", color: "#64748b" }}>{r.case_source}</td>
//                             </tr>
//                           ))}
//                         </tbody>
//                       </table>
...
//    module-level helper functions below (getEmptyForm, buildCheckRates,
//    buildCheckTats, caseToForm), which run before the API fetch
//    resolves and therefore can't read component state. Once the
//    check-types endpoint responds, checkTypeDefs is replaced with the
//    live list — including any custom check types (e.g. "Social Media")
//    an admin has added via Add Check Type — and everything in the
//    component body that renders check types reads from checkTypeDefs.
const FALLBACK_CHECK_TYPES = [
...
// ── Normalise a check_rates map coming from the API into the flat
//    { key: number } shape this form uses, applying the legacy key
//    aliases on the way through. Used both by the client auto-fill
//    below and by the edit-mode prefill. ───────────────────────────────
function normalizeCheckRates(raw) {
  const result = {};
  if (raw && typeof raw === "object") {
    Object.entries(raw).forEach(([key, val]) => {
      const mapped = CHECK_KEY_ALIASES[key] || key;
      result[mapped] = Number(val) || 0;
    });
  }
  return result;
}

// ── Same for check_tat. Accepts the { working_days, calendar_days }
//    shape the API sends, the { workingDays, calendarDays } shape the
//    form uses, or a legacy single number (applied to both fields).
function normalizeCheckTats(raw) {
  const result = {};
  if (raw && typeof raw === "object") {
    Object.entries(raw).forEach(([key, val]) => {
      const mapped = CHECK_KEY_ALIASES[key] || key;
      if (val && typeof val === "object") {
        ...
        const n = Number(val) || 0;
        result[mapped] = { workingDays: n, calendarDays: n };
      }
    });
  }
  return result;
...
function buildCheckRates(user) {
  if (user.role === "client" && user.checkRates && typeof user.checkRates === "object") {
    // FIX: this used to drop any rate whose key wasn't already in
    // DEFAULT_CHECK_RATES (`if (mapped in rates)`), so a custom check
    // type's contract rate was thrown away before the form ever saw it.
    // Merging instead, same as the TAT builder below.
    return { ...DEFAULT_CHECK_RATES, ...normalizeCheckRates(user.checkRates) };
  }
  return { ...DEFAULT_CHECK_RATES };
}
...
function buildCheckTats(user) {
  if (user.role === "client" && user.checkTat && typeof user.checkTat === "object") {
    // FIX: same `if (!(mapped in tats)) return;` filter as above —
    // a custom check type's contract TAT never made it into state.
    return { ...DEFAULT_CHECK_TAT, ...normalizeCheckTats(user.checkTat) };
  }
  return { ...DEFAULT_CHECK_TAT };
}
...
function getEmptyForm(user) {
  ...
  // FIX: this used to end with
  //   .filter(k => FALLBACK_CHECK_TYPES.some(ct => ct.key === k))
  // which silently discarded any agreed check that wasn't one of the 7
  // hardcoded built-ins. A client whose contract included a custom check
  // type (e.g. "Social Media") had it dropped from their preselection
  // every single time, with no indication why. The live catalogue is the
  // authority on which keys are valid, not the fallback list.
  const preselectedChecks = isClient && Array.isArray(user.agreedChecks)
    ? user.agreedChecks.map(k => CHECK_KEY_ALIASES[k] || k)
    : [];
...
// ── Bulk upload CSV parser ────────────────────────────────────────────────────
//
// FIX: the parsed rows were posted to /api/cases verbatim and never
// included candidate_dob, which that route validates as
// 'required|date'. Every bulk upload therefore 422'd — and because
// handleBulkSubmit only counted failures without reading the response
// body, the modal reported "N case(s) failed" with no reason. DOB is now
// parsed and treated as a required column, so a missing one is caught
// here with a row number instead of failing silently at the server.
function parseBulkCSV(text) {
  const lines = text.trim().split("\n").filter(Boolean);
  if (lines.length < 2) return { rows: [], errors: ["CSV must have a header row and at least one data row."] };
  const headers = lines[0].split(",").map(h => h.trim().toLowerCase().replace(/\s+/g, "_"));
  ...
  lines.slice(1).forEach((line, i) => {
    ...
    const dob = row.dob || row.candidate_dob || row.date_of_birth || "";

    if (!row.candidate_name && !row.candidate) { errors.push(`Row ${i + 2}: missing candidate name`); return; }
    if (!row.candidate_email && !row.email)    { errors.push(`Row ${i + 2}: missing email`); return; }
    if (!dob)                                  { errors.push(`Row ${i + 2}: missing date of birth (dob)`); return; }

    rows.push({
      candidate_name:  row.candidate_name || row.candidate,
      candidate_email: row.candidate_email || row.email,
      candidate_mobile: row.mobile || row.candidate_mobile || "",
      candidate_dob:   toDateInputValue(dob),
      position:        row.position || "",
      checks:          (row.checks || "employment").split("|").map(c => c.trim()).filter(Boolean),
      billing_mode:    row.billing_mode || "postpaid_client",
      client_name:     row.client_name || row.client || "",
      // Bulk rows default to client-sourced onboarding unless specified.
      case_source:     row.case_source || "client",
...
export default function AddCase() {
  ...
  //    Lets the candidate fill in check-specific forms and upload their
  //    supporting documents themselves, regardless of billing mode.
  //    (This now actually persists — candidate_portal_link was missing
  //    from BGVCase::$fillable and was being discarded on save.)
  const [candidatePortalLink, setCandidatePortalLink] = useState("");
  const [candidateLinkCopied, setCandidateLinkCopied] = useState(false);
  ...
  // ── Live check-type catalogue — starts from the 7 built-in fallback
  //    checks and is replaced by whatever the check-types endpoint
  //    returns. Everything that renders the check-types list in this form
  //    reads from checkTypeDefs, not the hardcoded array, so a newly
  //    added check type shows up here automatically.
  const [checkTypeDefs, setCheckTypeDefs] = useState(FALLBACK_CHECK_TYPES);
  ...
  // ── Fetch the live check-type catalogue.
  //
  //    FIX: this always called /api/check-types/all, which the backend
  //    guards with `if ($request->user()->role !== 'admin') return 403`.
  //    For a client or allocator the fetch 403'd, .json() produced
  //    { message: "Unauthorized" }, data.checkTypes was undefined, the
  //    `active.length === 0` early return fired, and checkTypeDefs stayed
  //    on the 7 hardcoded fallbacks forever — so non-admins never saw a
  //    single custom check type. The catch was empty, so nothing surfaced.
  //
  //    /api/check-types (no /all) is open to every authenticated role and
  //    already returns only active types, which is exactly what this form
  //    wants. Admins keep using /all so an inactive type still renders on
  //    a case that already uses it. Failures are now surfaced instead of
  //    swallowed.
  const [checkTypesError, setCheckTypesError] = useState("");

  useEffect(() => {
    let cancelled = false;
    const token = localStorage.getItem("token");
    const endpoint = isAdminUser ? "/api/check-types/all" : "/api/check-types";
    ...
    fetch(`${API_URL}${endpoint}`, {
      ...
      .catch(() => {
        if (!cancelled) setCheckTypesError("Couldn't load the check-type catalogue — showing built-in checks only.");
      });
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/cases/${encodeURIComponent(editCaseId)}`, {
      ...
      .then(data => {
        ...
        // FIX: the rate prefill used to sit inside `if (c.total_amount != null)`.
        // A case with a null total_amount (every candidate-sourced case
        // where nobody typed an amount) skipped the rate restore entirely,
        // so opening it for edit showed the client's default rates rather
        // than what was actually billed — and saving then overwrote the
        // real figures with those defaults. The two are unrelated; the
        // check_rates payload is authoritative on its own.
        if (c.check_rates && typeof c.check_rates === "object") {
          setRates(prev => ({ ...prev, ...normalizeCheckRates(c.check_rates) }));
        }

        // Keep admin-editable TAT in sync with whatever was actually set on
        // this case. Handles both the new { working_days, calendar_days }
        // shape and a legacy single number (applied to both fields).
        if (c.check_tat && typeof c.check_tat === "object") {
          setTats(prev => ({ ...prev, ...normalizeCheckTats(c.check_tat) }));
        }
  ...
  const totalAmount = form.checks.reduce((s, k) => s + (Number(rates[k]) || 0), 0);
  ...
  // Number(c.overall_tat ?? c.tat). Using the longest single check's
  // calendar days here (real elapsed time, not the sum) so that aggregate
  // stays meaningful. NOTE: this only began persisting once 'overall_tat'
  // was added to BGVCase::$fillable — it was being discarded before that.
  const overallTat = overallCalendarDaysValues.length > 0 ? Math.max(...overallCalendarDaysValues) : 0;
  ...
  // ── Only used by admin/allocator — clients have a locked
  //    clientId/clientName.
  //
  //    FIX: this used to set nothing but the client's name and billing
  //    mode, reading them off the /api/clients list payload. That list
  //    returns billing_mode but NOT the contract's agreed_checks /
  //    check_rates / check_tat, so picking a client left the check table
  //    showing default rates and blank TATs — the original symptom you
  //    reported. Fetching the full client record fills all three in.
  //
  //    The optimistic setForm runs first so the dropdown responds
  //    instantly; the fetch then refines it. If the fetch fails the
  //    optimistically-set values simply stand.
  //
  //    NOTE: GET /api/clients/{id} is currently admin-only on the
  //    backend, while this dropdown is also shown to allocators. For an
  //    allocator the fetch 403s and this returns quietly, leaving the
  //    rates/TAT unfilled. If allocators are meant to raise cases, widen
  //    that route's role check to match GET /api/clients.
  const handleClientChange = async (clientId) => {
    ...
    setForm(p => ({
      ...
      clientId,
      clientName:  client?.company_name || client?.name || "",
      billingMode: client?.billing_mode || client?.billingMode || p.billingMode,
    }));

    if (!clientId) return;

    // The list payload already carries these three for admin/allocator,
    // so apply them immediately rather than waiting on the round trip.
    if (client?.check_rates) setRates(prev => ({ ...prev, ...normalizeCheckRates(client.check_rates) }));
    if (client?.check_tat)   setTats(prev  => ({ ...prev, ...normalizeCheckTats(client.check_tat) }));
    if (Array.isArray(client?.agreed_checks) && client.agreed_checks.length > 0) {
      setForm(p => ({ ...p, checks: client.agreed_checks.map(k => CHECK_KEY_ALIASES[k] || k) }));
    }

    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_URL}/api/clients/${encodeURIComponent(clientId)}`, {
        headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
      });
      if (!res.ok) return; // 403 for allocators — keep the optimistic values
      const data = await res.json();
      const c = data.client || data;

      const agreed = Array.isArray(c.agreed_checks)
        ? c.agreed_checks.map(k => CHECK_KEY_ALIASES[k] || k)
        : [];

      setForm(p => ({
        ...p,
        billingMode: c.billing_mode || p.billingMode,
        checks: agreed.length > 0 ? agreed : p.checks,
      }));
      if (c.check_rates) setRates(prev => ({ ...prev, ...normalizeCheckRates(c.check_rates) }));
      if (c.check_tat)   setTats(prev  => ({ ...prev, ...normalizeCheckTats(c.check_tat) }));
    } catch {
      // Keep the optimistic values already set.
    }
  };

  // NOTE: both of these mint a fake placeholder URL client-side. They look
  // real in the UI and now persist to the database, but they point at
  // nothing — pay.bgvportal.in / portal.bgvportal.in aren't wired up. The
  // backend already has a real endpoint for the candidate one
  // (POST /api/cases/{caseId}/checks/{checkKey}/share-link, which creates
  // a CandidateLink with a token and expiry). Worth switching to it once
  // the case exists; leaving as-is since it's outside this fix.
  const generatePaymentLink = () => {
    const fake = `https://pay.bgvportal.in/c/${Math.random().toString(36).slice(2, 10)}`;
    setGeneratedLink(fake);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        // FIX: a 422 from Laravel puts the useful detail in data.errors,
        // not data.message — the old code showed the generic "Failed to
        // create case." and threw the actual field errors away, which is
        // why the mass-assignment and validation failures were so hard to
        // pin down from the UI.
        const fieldErrors = data.errors
          ? Object.values(data.errors).flat().join(" ")
          : "";
        setError(fieldErrors || data.message || (isEditMode ? "Failed to update case." : "Failed to create case."));
        return;
      }
  ...
  // FIX: bulk rows were POSTed with a client_name but no client_id. The
  // /api/cases route validates client_id as required_if:case_source,client,
  // so every client-sourced bulk row was rejected before it reached the
  // database. Resolving the id by matching client_name against the loaded
  // client list (case-insensitive), and reporting rows that can't be
  // matched rather than firing them off to fail.
  //
  // The old handler also reported success unconditionally — "N case(s)
  // were uploaded successfully" printed the full row count even when every
  // single one had failed, because it never read res.ok per row. Now it
  // counts actual successes and surfaces the server's reason.
  const resolveClientId = (clientName) => {
    if (isClientUser) return user.id;
    if (!clientName) return null;
    const match = clients.find(c =>
      (c.company_name || c.name || "").trim().toLowerCase() === clientName.trim().toLowerCase()
    );
  ...
  const handleBulkSubmit = async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled(
        bulkRows.map(async (row, i) => {
          const rowNum = i + 1;
          const clientId = resolveClientId(row.client_name);

          if (row.case_source === "client" && !clientId) {
            throw new Error(
              `Row ${rowNum} (${row.candidate_name}): client "${row.client_name || "—"}" not found. ` +
              `Use the exact company name as it appears in Clients, or set case_source to "candidate".`
            );
          }
          ...
          const res = await fetch(`${API_URL}/api/cases`, {
            ...
            body: JSON.stringify({
              ...row,
              client_id:   clientId,
              client_name: row.client_name || (row.case_source === "candidate" ? "Self (Candidate)" : ""),
              billing_mode: row.billing_mode || (row.case_source === "candidate" ? "prepaid_candidate" : ""),
            }),
          });
          ...
          if (!res.ok) {
            ...
            throw new Error(`Row ${rowNum} (${row.candidate_name}): ${detail}`);
          }
          ...
          <div className="dash-wrper">
            ...
            {checkTypesError && (
              <div style={{
                ...
                padding: "12px 16px", color: "#b45309", fontSize: "14px", margin: "12px 0"
              }}>
                {checkTypesError}
              </div>
            )}
            ...
            {!bulkDone ? (
              <>
                <div style={{ background: "#f0f4ff", borderRadius: "10px", padding: "16px", marginBottom: "20px", fontSize: "13px", color: "#475569" }}>
                  <strong>CSV Format:</strong> Upload a CSV file with the following columns:<br />
                  <code style={{ fontSize: "12px", background: "#e2e8f0", padding: "2px 6px", borderRadius: "4px" }}>
                    candidate_name, candidate_email, dob, mobile, position, checks, billing_mode, client_name, case_source
                  </code><br />
                  <span style={{ marginTop: "6px", display: "block" }}>
                    <code>dob</code> is required and must be <code>YYYY-MM-DD</code>.
                    For <code>checks</code>, separate multiple checks with <code>|</code> e.g. <code>employment|education|address</code>.
                    For <code>case_source</code>, use <code>client</code> or <code>candidate</code> (defaults to <code>client</code> if omitted).
                    <code>client_name</code> must match a company name in Clients exactly.
                  </span>
                </div>
                  ...
                  <div style={{ marginBottom: "16px" }}>
                    ...
                    <div style={{ maxHeight: "200px", overflowY: "auto", border: "1px solid #e2e8f0", borderRadius: "8px" }}>
                      <table style={{ width: "100%", fontSize: "12px", borderCollapse: "collapse" }}>
                        <thead>
                          <tr style={{ background: "#f8fafc" }}>
                            {["Candidate", "Email", "DOB", "Checks", "Billing", "Source"].map(h => (
                              <th key={h} style={{ padding: "8px 10px", textAlign: "left", fontWeight: 700,
                                borderBottom: "1px solid #e2e8f0", color: "#475569" }}>{h}</th>
                            ))}
                          </tr>
                        ...
                        <tbody>
                          {bulkRows.map((r, i) => (
                            <tr key={i} style={{ borderBottom: "1px solid #f1f5f9" }}>
                              <td style={{ padding: "7px 10px", color: "#1e293b" }}>{r.candidate_name}</td>
                              <td style={{ padding: "7px 10px", color: "#64748b" }}>{r.candidate_email}</td>
                              <td style={{ padding: "7px 10px", color: "#64748b" }}>{r.candidate_dob}</td>
                              <td style={{ padding: "7px 10px", color: "#64748b" }}>{r.checks?.join(", ")}</td>
                              <td style={{ padding: "7px 10px", color: "#64748b" }}>{r.billing_mode}</td>
                              <td style={{ padding: "7px 10px", color: "#64748b" }}>{r.case_source}</td>
                            </tr>
              ...
              /* FIX: this used to always show the green tick and claim every
                 row succeeded, using bulkRows.length as the count regardless
                 of what actually happened. Now the outcome reflects the real
                 per-row results and failures list their server-side reason. */
              <div style={{ textAlign: "center", padding: "24px 0" }}>
                <div style={{
                  width: "56px", height: "56px",
                  background: bulkSucceeded > 0 ? "#10b981" : "#dc2626",
                ...
                <h4 style={{ color: "#1e293b", marginBottom: "8px" }}>
                  {bulkSucceeded === 0
                    ? "Upload Failed"
                    : bulkErrors.length > 0 ? "Partially Uploaded" : "Upload Complete!"}
                </h4>
                <p style={{ color: "#64748b", fontSize: "14px", marginBottom: "16px" }}>
                  {bulkSucceeded} of {bulkRows.length} case(s) uploaded successfully.
                </p>

                {bulkErrors.length > 0 && (
                  <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: "8px",
                    padding: "12px", marginBottom: "20px", fontSize: "12px", color: "#dc2626",
                    textAlign: "left", maxHeight: "180px", overflowY: "auto" }}>
                    {bulkErrors.map((e, i) => <div key={i} style={{ marginBottom: "4px" }}>⚠ {e}</div>)}
                  </div>
                )}
...
const sharedStyles = `
  ...
  .ac-check-tile-top { display: flex; align-items: center; gap: 7px; font-size: 0.8rem; font-weight: 600; color: #334155; }
  .ac-check-active .ac-check-tile-top { color: #2b3b8c; }
  .ac-check-dot { width: 8px; height: 8px; border-radius: 50%; border: 2px solid #cbd5e1; flex-shrink: 0; display: inline-block; }
  .ac-check-active .ac-check-dot { border-color: #2b3b8c; background: #2b3b8c; }
  .ac-check-rate { font-size: 0.68rem; color: #94a3b8; font-weight: 600; text-align: right; white-space: nowrap; }
  .ac-check-active .ac-check-rate { color: #2b3b8c; }
  .ac-check-side { display: flex; align-items: center; gap: 6px; flex-shrink: 0; }
